...
不会发送 `<compacted-summary>`、compaction 内部说明、Standard 额外工具、runtime snapshot、AGENTS digest 或 skill catalog。
...
Epoch 内仍是 append-only history。摘要请求复用旧 system、tools 和 older-head prefix，只追加固定压缩指令。

Epoch boundary 后历史前部发生 replacement，旧 tail KV 无法直接接续；通常仍可能复用固定 system/tools。实际 cache hit、缓存寿命和计价由模型服务提供方决定。
...
为两个模式分别创建新 Session，并固定模型、reasoning effort、任务、仓库状态、权限和 compaction 配置。重点比较压缩后的重复探索、工具调用、错误恢复、token/cache usage 与最终任务质量。
...
当前测试覆盖 full-surface replacement、官方 tail boundary、reasoning A/B、tool record 转换、多次 compaction、失败回滚、subagent 默认排除和 preset 单变量一致性。
...
官方基线：[deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness)，commit `47f943859bef60e4160492346772ded9b24f765a`，主要参考官方 Minimal preset、`dsh-compaction-basic`、compaction service 和 Session projection。
