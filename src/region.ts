...
type SummarizedCompaction = PreparedCompaction & SummaryResult & {
  ...
  readonly landedSummary: ContentBlock[]
}
...
interface CompactionTransactionOptions {
  ...
  /** Last node summarized by the model; later selected nodes are recast verbatim as recent records. */
...
/** One complete epoch replacement plus the official retained-tail split. */
export interface CompactableEpochRange {
  readonly start: number
  readonly end: number
  readonly summaryEnd: number
}
 ...
 */
export function selectCompactableEpoch(
  session: Session,
  measurement: TokenMeasurement,
  retainTokens: number,
): CompactableEpochRange | null {
  const summaryRange = selectCompactableRange(session, measurement, retainTokens)
  if (summaryRange === null) return null
...
export async function compactSurfaceRegion(
  ...
  const summaryEndIdx = validateSummaryEnd(
    session,
    selection,
    options.summaryEnd ?? selection.end,
  )
...
async function summarizeCompaction(
  ...
  const landedSummary = frameSummary(
