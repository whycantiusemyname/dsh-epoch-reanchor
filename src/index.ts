...
import type { CompactableEpochRange } from './region.ts'
...
export class EpochCompactionEngine extends CompactionEngine {
  ...
  private _registerAutomaticCompaction(): void {
    ...
    const logResult = (result: CompactionResult, trigger: string): void => {
      ctx.logger.info(
        `epoch compaction (${trigger}): shadowed ${result.shadowedSeqs.length} surface nodes `
  ...
  override async compactIfNeeded(
    ...
    if (context === undefined) {
      throw new TargetPressureConfigError(
        targetKey,
        `dsh-epoch-reanchor: no context capacity for ${targetKey}; `
    ...
    try {
      return agent.runMaintenance(async (agentSignal) => {
        ...
        try {
          ...
          const range = selectCompactableEpoch(
            agent.session,
  ...
  private compactEpochRange(
    range: CompactableEpochRange,
    agent: Agent,
    signal?: AbortSignal,
  ): Promise<CompactionResult> {
    return compactSurfaceRegion(
