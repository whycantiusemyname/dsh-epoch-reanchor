...
import { includesSessionInEpochMode, isLocalSubagentSession, isSubagentEpochMode, } from "./subagent-mode.js";
...
function withPersona(message, persona) {
...
function taskMessageIndex(messages) {
...
function eligibleChild(agent, mode) {
...
export function apply(ctx, config = {}) {
    ...
    const pendingPersona = new WeakMap();
    ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
        const assembled = await next();
        const agent = context.agent;
        if (agent === undefined || !eligibleChild(agent, mode))
            return assembled;
...
//# sourceMappingURL=subagent-epoch.js.map