 ...
 */
export function serializeRecentTail(messages, includeReasoning) {
    const output = [];
...
export function frameSummary(summary, recentTail = [], includeReasoning = false) {
    const output = [{ type: 'text', text: `${HANDOFF_PREFIX}\n` }, ...summary];
    if (recentTail.length > 0) {
        appendText(output, `\n\n${RECENT_RECORDS_HEADING}\n\n`);
        for (const block of serializeRecentTail(recentTail, includeReasoning))
            output.push(block);
    }
    appendText(output, `\n${HANDOFF_SUFFIX}`);
    return output;
