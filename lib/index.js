/**
 * Replay-aware trajectory-epoch compaction backend.
 *
 * Derived from the official `@deepseek-ai/dsh-compaction-basic` provider.
 * @module dsh-epoch-reanchor
 */
...
import { assertNoActiveCompaction, compactSurfaceRegion, selectCompactableEpoch, } from "./region.js";
import { summarizeWithLlm } from "./summarizer.js";
import { includesSessionInEpochMode } from "./subagent-mode.js";
export { frameSummary, serializeRecentTail } from "./summarizer.js";
export { selectCompactableEpoch, selectCompactableRange } from "./region.js";
export { deferredPersonaFooter, deferredPersonaOf, MINIMAL_PERSONA, ROLE_GUIDANCE_HEADING, stripDeferredPersonaFooter, } from "./subagent-epoch.js";
export { includesSessionInEpochMode, isFreshLocalSubagent, isLocalSubagentSession, resolveSubagentEpochMode, } from "./subagent-mode.js";
...
const subagentModeSchema = z.union(['off', 'fresh', 'all']);
 ...
 * token meter.
 */
export class EpochCompactionEngine extends CompactionEngine {
    ...
    _registerAutomaticCompaction() {
        ...
        const logResult = (result, trigger) => {
            ctx.logger.info(`epoch compaction (${trigger}): shadowed ${result.shadowedSeqs.length} surface nodes `
    ...
    async compactIfNeeded(agent, trigger, signal) {
        ...
        if (trigger === 'context-overflow') {
            ...
            const range = selectCompactableEpoch(agent.session, measurement, 0);
            if (range === null)
                return null;
            return this.compactEpochRange(range, agent, signal);
        }
        const context = (await this.ctx.llm.resolveModelInfo(target.provider, target.model, signal)).context;
        assertNoActiveCompaction(agent.session, 'automatic pressure compaction');
        const targetKey = `${target.provider}/${target.model}`;
        if (context === undefined) {
            throw new TargetPressureConfigError(targetKey, `dsh-epoch-reanchor: no context capacity for ${targetKey}; `
        ...
        for (let attempt = 0; attempt <= spec.compactionRetries; attempt += 1) {
            const range = selectCompactableEpoch(agent.session, measurement, spec.retainTokens);
            if (range === null) {
            ...
            result = await this.compactEpochRange(range, agent, signal);
            measurement = meter.measure(agent.session);
            if (measurement.totalTokens < spec.thresholdTokens)
                return result;
        }
        ...
        try {
            return agent.runMaintenance(async (agentSignal) => {
                ...
                try {
                    ...
                    const range = selectCompactableEpoch(agent.session, this.ctx.tokenMeter.measure(agent.session), 0);
                    if (range === null)
                        return null;
                    return await compactSurfaceRegion(this.regionDependencies(), agent.session, range.start, range.end, agent, {
                        owner: null,
                        stability: 'selected-span',
                        summaryEnd: range.summaryEnd,
    ...
    /** Replace the complete current surface while summarizing only the official older head. */
    compactEpochRange(range, agent, signal) {
        return compactSurfaceRegion(this.regionDependencies(), agent.session, range.start, range.end, agent, {
            owner: 'current-turn',
    ...
    regionDependencies() {
        return {
            ...
            includeTailReasoning: this.config.includeTailReasoning,
            summarize: (input, owner, abort) => this.summarize(input, owner, abort),
        };
    }
}
export default EpochCompactionEngine;
