 ...
 *
 * Derived from `@deepseek-ai/dsh-compaction-basic/region`.
 * @module dsh-epoch-reanchor/region
 */
...
import { deferredPersonaOf } from "./subagent-epoch.js";
/**
...
/**
 * Preserve the official head/tail selection, but replace the whole current
 * surface so the retained tail can be recast inside one fresh user task.
 */
export function selectCompactableEpoch(session, measurement, retainTokens) {
    const summaryRange = selectCompactableRange(session, measurement, retainTokens);
    ...
    if (start === undefined || end === undefined)
        return null;
    return { start, end, summaryEnd: summaryRange.end };
}
/**
...
export async function compactSurfaceRegion(dependencies, session, start, end, agent, options, signal) {
    ...
    const summaryEndIdx = validateSummaryEnd(session, selection, options.summaryEnd ?? selection.end);
    ...
    let stage = 'summary';
    try {
        const prepared = prepareCompaction(dependencies, session, selection, summaryEndIdx);
...
/** Validate the model-summary/tail boundary inside the complete replacement span. */
function validateSummaryEnd(session, selection, summaryEnd) {
    const summaryEndIdx = session.surface.nodes.indexOf(summaryEnd);
    if (summaryEndIdx < selection.startIdx || summaryEndIdx > selection.endIdx) {
        throw new Error(`compaction: summary end seq ${summaryEnd} is outside the replacement range`);
    }
    if (!toolPairingBalancedAfter(session, summaryEnd)) {
        throw new Error(`compaction: summary end seq ${summaryEnd} would split a tool-call/result pair`);
    }
    return summaryEndIdx;
}
/** Snapshot pricing and replay input for a validated surface range. */
function prepareCompaction(dependencies, session, selection, summaryEndIdx) {
    ...
    const summaryCount = summaryEndIdx - selection.startIdx + 1;
    const summarySeqs = selection.shadowedSeqs.slice(0, summaryCount);
    const tailSeqs = selection.shadowedSeqs.slice(summaryCount);
    const events = session.events;
    const tailMessages = tailSeqs
    ...
    return {
        ...
        input: buildSummarizationInput(session, summarySeqs),
        tailMessages,
    };
}
...
async function summarizeCompaction(dependencies, prepared, agent, compactionId, sourceCommandId, signal) {
    ...
    const landedSummary = frameSummary(summaryResult.summary, prepared.tailMessages, dependencies.includeTailReasoning, deferredPersonaOf(agent.session));
...
function commitCompactionBody(session, startEvent, summarized) {
    const { start, end, shadowedSeqs, shadowedTokenCount, landedSummary, provider, model, maxTokens, usage, checkpointMessage, } = summarized;
 ...
 * region: its system prompt and tool schemas, then the older head selected
 * for model summarization. The summarizer appends only the compaction
