/**
 * Load-time validation and routed-model policy resolution for epoch compaction.
 *
 * Derived from `@deepseek-ai/dsh-compaction-basic/config`.
 * @module dsh-epoch-reanchor/config
 */
import { deepFreeze } from '@deepseek-ai/dsh-llm';
import { resolveSubagentEpochMode } from "./subagent-mode.js";
...
export function resolveConfig(config = {}) {
    ...
    if (config.includeTailReasoning !== undefined
        && typeof config.includeTailReasoning !== 'boolean') {
        throw new Error('BasicCompactionConfig: includeTailReasoning must be a boolean');
    }
    const subagentMode = resolveSubagentEpochMode(config.subagentMode, config.includeSubagents, 'BasicCompactionConfig');
